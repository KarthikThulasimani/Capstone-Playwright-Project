...
export class CheckoutPage {
    ...
    // Enter Bank Details
    async enterBankDetails(bankName: string,accountName: string,accountNumber: string) {
    ...
    async enterCreditCardDetails(creditCardNumber: string,expirationDate: string, cvv: string,cardHolderName: string) {
    ...
    async enterGiftCardDetails(giftCardNumber: string,validationCode: string) {
    ...
    await expect(this.locators.paymentSuccessMessage).toBeVisible({timeout: 10000});
    await expect(this.locators.paymentSuccessMessage).toHaveText('Payment was successful');
    }
